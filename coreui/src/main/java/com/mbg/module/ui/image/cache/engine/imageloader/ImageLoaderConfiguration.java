...
import com.mbg.module.ui.image.cache.common.BitmapProcessor;
import com.mbg.module.ui.image.cache.common.FileNameGenerator;
import com.mbg.module.ui.image.cache.common.FlushedInputStream;
import com.mbg.module.ui.image.cache.common.QueueProcessingType;
import com.mbg.module.ui.image.cache.decode.ImageDecoder;
import com.mbg.module.ui.image.cache.disk.DiskCache;
import com.mbg.module.ui.image.cache.download.ImageDownloader;
...
import com.mbg.module.ui.image.cache.memory.MemoryCache;
import com.mbg.module.ui.image.cache.memory.impl.FuzzyKeyMemoryCache;
import com.mbg.module.ui.image.cache.model.ImageSize;
import com.mbg.module.ui.image.cache.util.MemoryCacheUtils;
...
public final class ImageLoaderConfiguration {
	...
	private final BitmapProcessor processorForDiskCache;

	private final Executor taskExecutor;
	private final Executor taskExecutorForCachedImages;
	private final boolean customExecutor;
	private final boolean customExecutorForCachedImages;
	...
	public ImageDownloader getNetworkDeniedDownloader() {
		return networkDeniedDownloader;
	}

	public ImageDownloader getSlowNetworkDownloader() {
	...
	public BitmapProcessor getProcessorForDiskCache() {
		return processorForDiskCache;
	}
	...
	public boolean isCustomExecutorForCachedImages() {
		return customExecutorForCachedImages;
	}
	...
	/**
	 * Creates default configuration for { ImageLoader} <br />
	 * <b>Default values:</b>
	 * <ul>
	 ...
	 * <li>threadPoolSize = { Builder#DEFAULT_THREAD_POOL_SIZE this}</li>
	 * <li>threadPriority = { Builder#DEFAULT_THREAD_PRIORITY this}</li>
	 * <li>allow to cache different sizes of image in memory</li>
	 * <li>memoryCache = { DefaultConfigurationFactory#createMemoryCache(Context, int)}</li>
	 * <li>diskCache = { com.nostra13.universalimageloader.cache.disc.impl.UnlimitedDiskCache}</li>
	 * <li>imageDownloader = { DefaultConfigurationFactory#createImageDownloader(Context)}</li>
	 * <li>imageDecoder = { DefaultConfigurationFactory#createImageDecoder(boolean)}</li>
	 * <li>diskCacheFileNameGenerator = { DefaultConfigurationFactory#createFileNameGenerator()}</li>
	 * <li>defaultDisplayImageOptions = { DisplayImageOptions#createSimple() Simple options}</li>
	 * <li>tasksProcessingOrder = { QueueProcessingType#FIFO}</li>
	...
	public static class Builder {
		 ...
		 *                                     an image to { android.graphics.Bitmap Bitmap}. <b>Default value - device's screen width</b>
		 * @param maxImageHeightForMemoryCache Maximum image height which will be used for memory saving during decoding
		 *                                     an image to { android.graphics.Bitmap Bitmap}. <b>Default value</b> - device's screen height
		 */
		...
		/**
		 * @deprecated Use
		 * { #diskCacheExtraOptions(int, int, com.nostra13.universalimageloader.core.process.BitmapProcessor)}
		 * instead
		 */
		@Deprecated
		 ...
		 * @param processorForDiskCache      null-ok; {plain BitmapProcessor Bitmap processor} which process images before saving them in disc cache
		 */
		...
		/**
		 * Sets custom {plain Executor executor} for tasks of loading and displaying images.<br />
		 * <br />
		 * <b>NOTE:</b> If you set custom executor then following configuration options will not be considered for this
		 * executor:
		 * <ul>
		 * <li>{ #threadPoolSize(int)}</li>
		 * <li>{ #threadPriority(int)}</li>
		 * <li>{ #tasksProcessingOrder(QueueProcessingType)}</li>
		 * </ul>
		 *
		...
		public Builder taskExecutor(Executor executor) {
			if (threadPoolSize != DEFAULT_THREAD_POOL_SIZE || threadPriority != DEFAULT_THREAD_PRIORITY || tasksProcessingType != DEFAULT_TASK_PROCESSING_TYPE) {
				LogUtils.w(WARNING_OVERLAP_EXECUTOR);
			}
		...
		/**
		 * Sets custom {plain Executor executor} for tasks of displaying <b>cached on disk</b> images (these tasks
		 * are executed quickly so UIL prefer to use separate executor for them).<br />
		 * <br />
		 * If you set the same executor for {plain #taskExecutor(Executor) general tasks} and
		 ...
		 * Default value - { #DEFAULT_THREAD_POOL_SIZE this}
		 */
		...
		/**
		 * Sets the priority for image loading threads. Should be <b>NOT</b> greater than { Thread#MAX_PRIORITY} or
		 * less than { Thread#MIN_PRIORITY}<br />
		 * Default value - { #DEFAULT_THREAD_PRIORITY this}
		 */
		...
		/**
		 * When you display an image in a small { android.widget.ImageView ImageView} and later you try to display
		 * this image (from identical URI) in a larger { android.widget.ImageView ImageView} so decoded image of
		...
		/**
		 * Sets maximum memory cache size for { android.graphics.Bitmap bitmaps} (in bytes).<br />
		 * Default value - 1/8 of available app memory.<br />
		 * <b>NOTE:</b> If you use this method then
		 * { com.nostra13.universalimageloader.cache.memory.impl.LruMemoryCache LruMemoryCache} will be used as
		 * memory cache. You can use { #memoryCache(MemoryCache)} method to set your own implementation of
		 * { MemoryCache}.
		 */
		public Builder memoryCacheSize(int memoryCacheSize) {
			...
			if (memoryCache != null) {
				LogUtils.w(WARNING_OVERLAP_MEMORY_CACHE);
			}
		...
		/**
		 * Sets maximum memory cache size (in percent of available app memory) for { android.graphics.Bitmap
		 * bitmaps}.<br />
		 * Default value - 1/8 of available app memory.<br />
		 * <b>NOTE:</b> If you use this method then
		 * { com.nostra13.universalimageloader.cache.memory.impl.LruMemoryCache LruMemoryCache} will be used as
		...
		/**
		 * Sets memory cache for { android.graphics.Bitmap bitmaps}.<br />
		 * Default value - { com.nostra13.universalimageloader.cache.memory.impl.LruMemoryCache LruMemoryCache}
		 ...
		 * <b>NOTE:</b> If you use this method then
		 * { com.nostra13.universalimageloader.cache.disc.impl.ext.LruDiskCache LruDiskCache}
		 * will be used as disk cache. You can use { #diskCache(DiskCache)} method for introduction your own
		...
		public Builder diskCacheSize(int maxCacheSize) {
			...
			if (diskCache != null) {
				LogUtils.w(WARNING_OVERLAP_DISK_CACHE_PARAMS);
			}
		...
		public Builder diskCacheFileNameGenerator(FileNameGenerator fileNameGenerator) {
			if (diskCache != null) {
				LogUtils.w(WARNING_OVERLAP_DISK_CACHE_NAME_GENERATOR);
			}
		 ...
		 * Default value - { com.nostra13.universalimageloader.cache.disc.impl.UnlimitedDiskCache
		 ...
		 * <ul>
		 * <li>{ #diskCacheSize(int)}</li>
		 * <li>{ #diskCacheFileCount(int)}</li>
		 * <li>{ #diskCacheFileNameGenerator(FileNameGenerator)}</li>
		 * </ul>
		 */
		 ...
		 * Default value -
		 * { com.nostra13.universalimageloader.core.DefaultConfigurationFactory#createImageDownloader(Context)
		 ...
		 * Default value -
		 * { com.nostra13.universalimageloader.core.DefaultConfigurationFactory#createImageDecoder(boolean)
		...
		/**
		 * Sets default {plain DisplayImageOptions display image options} for image displaying. These options will
		 * be used for every {plain ImageLoader#displayImage(String, android.widget.ImageView) image display call}
		 * without passing custom {plain DisplayImageOptions options}<br />
		 * Default value - { DisplayImageOptions#createSimple() Simple options}
		 */
		 ...
		 * Consider { com.nostra13.universalimageloader.utils.L#disableLogging()} to disable
	...
	/**
	 * Decorator. Prevents downloads from network (throws { IllegalStateException exception}).<br />
	 ...
	 * using { com.nostra13.universalimageloader.core.assist.FlushedInputStream}.
	 *
	 * @author Sergey Tarasevich (nostra13[at]gmail[dot]com)
	 * @since 1.8.1
	 */
