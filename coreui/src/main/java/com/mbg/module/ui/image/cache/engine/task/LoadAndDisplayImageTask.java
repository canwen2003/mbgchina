 ...
 *******************************************************************************/
package com.mbg.module.ui.image.cache.engine.task;
...
import com.mbg.module.ui.image.cache.common.IoUtils;
...
import com.mbg.module.ui.image.cache.decode.ImageDecodingInfo;
...
import com.mbg.module.ui.image.cache.engine.FailReason;
import com.mbg.module.ui.image.cache.engine.imageloader.ImageLoaderConfiguration;
import com.mbg.module.ui.image.cache.engine.imageloader.ImageLoaderEngine;
import com.mbg.module.ui.image.cache.engine.imageloader.ImageLoadingInfo;
 ...
 * display it in {com.nostra13.universalimageloader.core.imageaware.ImageAware} using { DisplayBitmapTask}.
 *
 ...
 * @since 1.3.1
 */
public final class LoadAndDisplayImageTask implements Runnable, CopyListener {
	...
	public LoadAndDisplayImageTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler) {
		...
		downloader = configuration.getDownloader();
		networkDeniedDownloader = configuration.getNetworkDeniedDownloader();
		slowNetworkDownloader = configuration.getSlowNetworkDownloader();
		decoder = configuration.getDecoder();
	...
	public void run() {
		...
		LogUtils.d(LOG_START_DISPLAY_IMAGE_TASK, memoryCacheKey);
		if (loadFromUriLock.isLocked()) {
			LogUtils.d(LOG_WAITING_FOR_IMAGE_LOADED, memoryCacheKey);
		}
		...
		try {
			...
			bmp = configuration.getMemoryCache().get(memoryCacheKey);
			if (bmp == null || bmp.isRecycled()) {
				...
				if (options.shouldPreProcess()) {
					LogUtils.d(LOG_PREPROCESS_IMAGE, memoryCacheKey);
					bmp = options.getPreProcessor().process(bmp);
					if (bmp == null) {
						LogUtils.e(ERROR_PRE_PROCESSOR_NULL, memoryCacheKey);
					}
				}

				if (bmp != null && options.isCacheInMemory()) {
					LogUtils.d(LOG_CACHE_IMAGE_IN_MEMORY, memoryCacheKey);
					configuration.getMemoryCache().put(memoryCacheKey, bmp);
				}
			} else {
				loadedFrom = LoadedFrom.MEMORY_CACHE;
				LogUtils.d(LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING, memoryCacheKey);
			}

			if (bmp != null && options.shouldPostProcess()) {
				LogUtils.d(LOG_POSTPROCESS_IMAGE, memoryCacheKey);
				bmp = options.getPostProcessor().process(bmp);
				if (bmp == null) {
					LogUtils.e(ERROR_POST_PROCESSOR_NULL, memoryCacheKey);
				}
			}
			checkTaskNotActual();
			checkTaskInterrupted();
	...
	private boolean waitIfPaused() {
		...
		if (pause.get()) {
			synchronized (engine.getPauseLock()) {
				if (pause.get()) {
					LogUtils.d(LOG_WAITING_FOR_RESUME, memoryCacheKey);
					try {
						engine.getPauseLock().wait();
					} catch (InterruptedException e) {
						LogUtils.e(LOG_TASK_INTERRUPTED, memoryCacheKey);
						return true;
					}
					LogUtils.d(LOG_RESUME_AFTER_PAUSE, memoryCacheKey);
				}
			}
		}
	...
	private boolean delayIfNeed() {
		if (options.shouldDelayBeforeLoading()) {
			LogUtils.d(LOG_DELAY_BEFORE_LOADING, options.getDelayBeforeLoading(), memoryCacheKey);
			try {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		Bitmap bitmap = null;
		try {
			File imageFile = configuration.getDiskCache().get(uri);
			if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
				LogUtils.d(LOG_LOAD_IMAGE_FROM_DISK_CACHE, memoryCacheKey);
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				LogUtils.d(LOG_LOAD_IMAGE_FROM_NETWORK, memoryCacheKey);
				loadedFrom = LoadedFrom.NETWORK;

				String imageUriForDecoding = uri;
				if (options.isCacheOnDisk() && tryCacheImageOnDisk()) {
					imageFile = configuration.getDiskCache().get(uri);
					if (imageFile != null) {
	...
	private boolean tryCacheImageOnDisk() throws TaskCancelledException {
		LogUtils.d(LOG_CACHE_IMAGE_ON_DISK, memoryCacheKey);
		...
		try {
			...
			if (loaded) {
				...
				if (width > 0 || height > 0) {
					LogUtils.d(LOG_RESIZE_CACHED_IMAGE_FILE, memoryCacheKey);
	...
	private boolean downloadImage() throws IOException {
		...
		if (is == null) {
			LogUtils.e(ERROR_NO_IMAGE_STREAM, memoryCacheKey);
			return false;
		} else {
			try {
				return configuration.getDiskCache().save(uri, is, this);
			} finally {
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
		...
		boolean saved = false;
		File targetFile = configuration.getDiskCache().get(uri);
		if (targetFile != null && targetFile.exists()) {
			...
			if (bmp != null && configuration.getProcessorForDiskCache() != null) {
				LogUtils.d(LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISK, memoryCacheKey);
				bmp = configuration.getProcessorForDiskCache().process(bmp);
				if (bmp == null) {
					LogUtils.e(ERROR_PROCESSOR_FOR_DISK_CACHE_NULL, memoryCacheKey);
				}
			}
			if (bmp != null) {
				saved = configuration.getDiskCache().save(uri, bmp);
				bmp.recycle();
			}
		}
		return saved;
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				if (options.shouldShowImageOnFail()) {
					imageAware.setImageDrawable(options.getImageOnFail(configuration.getResources()));
				}
	...
	private boolean isViewCollected() {
		if (imageAware.isCollected()) {
			LogUtils.d(LOG_TASK_CANCELLED_IMAGEAWARE_COLLECTED, memoryCacheKey);
			return true;
		}
		return false;
	}
	...
	private boolean isViewReused() {
		...
		if (imageAwareWasReused) {
			LogUtils.d(String.format(LOG_TASK_CANCELLED_IMAGEAWARE_REUSED, memoryCacheKey));
			return true;
		}
		return false;
	}
	...
	private boolean isTaskInterrupted() {
		if (Thread.interrupted()) {
			LogUtils.d(String.format(LOG_TASK_INTERRUPTED, memoryCacheKey));
			return true;
		}
		return false;
	}
