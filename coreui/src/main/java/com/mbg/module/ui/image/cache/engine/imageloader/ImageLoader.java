...
import com.mbg.module.ui.image.cache.common.LoadedFrom;
import com.mbg.module.ui.image.cache.common.ViewScaleType;
...
import com.mbg.module.ui.image.cache.engine.listener.SimpleImageLoadingListener;
...
import com.mbg.module.ui.image.cache.imageaware.ImageViewAware;
import com.mbg.module.ui.image.cache.imageaware.NonViewAware;
...
import com.mbg.module.ui.image.cache.util.ImageSizeUtils;
import com.mbg.module.ui.image.cache.util.MemoryCacheUtils;


/**
 * Singletone for image loading and displaying at { ImageView ImageViews}<br />
 * <b>NOTE:</b> { #init(ImageLoaderConfiguration)} method must be called before any other method.
 *
...
public class ImageLoader {
	 ...
	 * If configurations was set before ( { #isInited()} == true) then this method does nothing.<br />
	 * To force initialization with new configuration you should { #destroy() destroy ImageLoader} at first.
	 *
	 * @param configuration { ImageLoaderConfiguration ImageLoader configuration}
	 *  IllegalArgumentException if <b>configuration</b> parameter is null
	 */
	...
	/**
	 * Returns <b>true</b> - if ImageLoader { #init(ImageLoaderConfiguration) is initialized with
	 ...
	 * Default { DisplayImageOptions display image options} from { ImageLoaderConfiguration
	 * configuration} will be used.<br />
	 * <b>NOTE:</b> { #init(ImageLoaderConfiguration)} method must be called before this method call
	 *
	 * @param uri        Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
	 * @param imageAware { com.nostra13.universalimageloader.core.imageaware.ImageAware Image aware view}
	 *                   which should display image
	 *  IllegalStateException    if { #init(ImageLoaderConfiguration)} method wasn't called before
	 *  IllegalArgumentException if passed <b>imageAware</b> is null
	 */
	 ...
	 * @param imageAware { com.nostra13.universalimageloader.core.imageaware.ImageAware Image aware view}
	 *                   which should display image
	 * @param listener   { ImageLoadingListener Listener} for image loading process. Listener fires events on
	 ...
	 * @param imageAware { com.nostra13.universalimageloader.core.imageaware.ImageAware Image aware view}
	 *                   which should display image
	 * @param options    { com.nostra13.universalimageloader.core.DisplayImageOptions Options} for image
	 *                   decoding and displaying. If <b>null</b> - default display image options
	 *                   { ImageLoaderConfiguration.Builder#defaultDisplayImageOptions(DisplayImageOptions)
	 ...
	 *                         { ImageLoaderConfiguration.Builder#defaultDisplayImageOptions(DisplayImageOptions)
	 *                         from configuration} will be used.
	 * @param listener         { ImageLoadingListener Listener} for image loading process. Listener fires
	 *                         events on UI thread if this method is called on UI thread.
	 * @param progressListener { com.nostra13.universalimageloader.core.listener.ImageLoadingProgressListener
	 *                         Listener} for image loading progress. Listener fires events on UI thread if this method
	 *                         is called on UI thread. Caching on disk should be enabled in
	 *                         { com.nostra13.universalimageloader.core.DisplayImageOptions options} to make
	 *                         this listener work.
	 *  IllegalStateException    if { #init(ImageLoaderConfiguration)} method wasn't called before
	 *  IllegalArgumentException if passed <b>imageAware</b> is null
	 */
	 ...
	 *                         { ImageLoaderConfiguration.Builder#defaultDisplayImageOptions(DisplayImageOptions)
	 *                         from configuration} will be used.
	 * @param targetSize       { ImageSize} Image target size. If <b>null</b> - size will depend on the view
	 * @param listener         { ImageLoadingListener Listener} for image loading process. Listener fires
	 *                         events on UI thread if this method is called on UI thread.
	 * @param progressListener { com.nostra13.universalimageloader.core.listener.ImageLoadingProgressListener
	 *                         Listener} for image loading progress. Listener fires events on UI thread if this method
	...
	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
		...
		if (options == null) {
			options = configuration.getDefaultDisplayImageOptions();
		}
		...
		if (TextUtils.isEmpty(uri)) {
			...
			if (options.shouldShowImageForEmptyUri()) {
				imageAware.setImageDrawable(options.getImageForEmptyUri(configuration.getResources()));
			} else {
		...
		Bitmap bmp = configuration.getMemoryCache().get(memoryCacheKey);
		if (bmp != null && !bmp.isRecycled()) {
			LogUtils.d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE+ memoryCacheKey);
			...
			if (options.shouldShowImageOnLoading()) {
				imageAware.setImageDrawable(options.getImageOnLoading(configuration.getResources()));
	 ...
	 *  IllegalStateException    if { #init(ImageLoaderConfiguration)} method wasn't called before
	 *  IllegalArgumentException if passed <b>imageView</b> is null
	 */
	 ...
	 * { ImageLoadingListener#onLoadingComplete(String, View, Bitmap)} callback}.
	 * <br />
	 * <b>NOTE:</b> { #init(ImageLoaderConfiguration)} method must be called before this method call
	 *
	 * @param uri      Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
	 * @param listener { ImageLoadingListener Listener} for image loading process. Listener fires events on UI
	 ...
	 *
	 * @param uri             Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
	 * @param targetImageSize Minimal size for { Bitmap} which will be returned in
	 ...
	 *
	 * @param uri             Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
	 * @param targetImageSize Minimal size for { Bitmap} which will be returned. Downloaded image will be decoded
	 *                        and scaled to { Bitmap} of the size which is <b>equal or larger</b> (usually a bit
	 *                        larger) than incoming targetImageSize.
	 * @return Result image Bitmap. Can be <b>null</b> if image loading/decoding was failed or cancelled.
	 *  IllegalStateException if { #init(ImageLoaderConfiguration)} method wasn't called before
	 ...
	 *
	 *  IllegalStateException if configuration wasn't initialized
	 */
	...
	public MemoryCache getMemoryCache() {
		checkConfiguration();
		return configuration.getMemoryCache();
	}
	...
	public DiskCache getDiskCache() {
		checkConfiguration();
		return configuration.getDiskCache();
	}
	 ...
	 * { com.nostra13.universalimageloader.core.imageaware.ImageAware ImageAware}.
	 *
	 * @param imageAware { com.nostra13.universalimageloader.core.imageaware.ImageAware ImageAware} for
	 ...
	 * { ImageView ImageView}.
	 *
	 * @param imageView { ImageView ImageView} for which display task will be cancelled
	 */
	 ...
	 * { ImageLoadingListener#onLoadingFailed(String, View, FailReason)} callback will be fired with
	...
	/**
	 * Pause ImageLoader. All new "load&display" tasks won't be executed until ImageLoader is { #resume() resumed}.
	 * <br />
	...
	/**
	 * { #stop() Stops ImageLoader} and clears current configuration. <br />
	 * You can { #init(ImageLoaderConfiguration) init} ImageLoader with new configuration after calling this
	 * method.
	 */
	public void destroy() {
		if (configuration != null) LogUtils.d(LOG_DESTROY);
		stop();
